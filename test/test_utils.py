...
def test_mount_config(mock_nfs_mount):
    ...
    assert shutil.which(BRTConfig.binvol)
    assert shutil.which(Config.bioformats2raw)
    assert shutil.which(Config.brt_binary)
    assert shutil.which(DMConfig.dm2mrc_loc)
    assert shutil.which(BRTConfig.clip_loc)
    # assert os.path.exists(SEMConfig.convert_loc) # uses gm instead (graphicsmagick)
    assert shutil.which(Config.header_loc)
    assert shutil.which(Config.mrc2tif_loc)
    assert shutil.which(Config.newstack_loc)
    assert shutil.which(SEMConfig.tif2mrc_loc)
    assert shutil.which(SEMConfig.xfalign_loc)
    assert shutil.which(SEMConfig.xftoxg_loc)
