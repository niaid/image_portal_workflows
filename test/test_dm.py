...
def test_dm4_conv(mock_nfs_mount):
    from em_workflows.dm_conversion.flow import main_flow
...
def test_single_file_no_ext_not_found_gens_exception(mock_nfs_mount, caplog):
    from em_workflows.dm_conversion.flow import main_flow
    ...
    assert state.is_failed()
    assert "not contain anything to process" in caplog.text
